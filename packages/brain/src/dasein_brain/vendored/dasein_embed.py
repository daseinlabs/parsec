"""Dasein in-cluster embedding client.

Calls the embedding model we ALREADY run on GPU in the dasein cluster (the `dasein-embed` API pod,
...
DASEIN-PATCH: transport swapped for the brain service. The reference client resolved the GPU pod's
...
posts JSON to the CPU proxy Service instead: POST DASEIN_EMBED_URL (default
...
# DASEIN-PATCH: JSON proxy endpoint replaces k8s pod discovery + binary /encode_bin.
EMBED_URL = os.environ.get("DASEIN_EMBED_URL",
                           "http://dasein-embed.default.svc.cluster.local/embed")
MODEL_ID = os.environ.get("DASEIN_EMBED_MODEL", "bge-large-en-v1.5")
...
BATCH = int(os.environ.get("DASEIN_EMBED_BATCH", "512"))   # 512 amortizes the GPU better than 256 on
...
class DaseinEmbedClient:
    ...
    def _embed_batch(self, texts: list[str]) -> np.ndarray:
        # DASEIN-PATCH: JSON /embed on the CPU proxy (embed-protocol §2a) — response
