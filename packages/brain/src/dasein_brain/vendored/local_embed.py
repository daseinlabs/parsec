"""In-process bge-large-en-v1.5 embedder (DASEIN_EMBED_BACKEND=local).
...
usage (model card + the cluster's SentenceTransformer config):
  ...
  * NO query instruction. The cluster JSON endpoint (dasein_embed.py) posts
...
# HF id or a baked local dir (Cloud Run sets DASEIN_EMBED_MODEL_DIR=/embed-model).
_MODEL = os.environ.get("DASEIN_EMBED_MODEL_DIR", "BAAI/bge-large-en-v1.5")
_MAXTOK = int(os.environ.get("DASEIN_EMBED_MAXTOK", "512"))
_BATCH = int(os.environ.get("DASEIN_EMBED_BATCH", "512"))


class LocalEmbedClient:
    """In-process encoder; same .embed(texts, as_query) surface as DaseinEmbedClient."""

    def __init__(self, cfg: dict | None = None):
        import torch
        from transformers import AutoModel, AutoTokenizer

        self.device = os.environ.get("DASEIN_EMBED_DEVICE") or (
