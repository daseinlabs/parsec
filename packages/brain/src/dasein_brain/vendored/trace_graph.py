...
VENDORED TRIM (dasein-brain serving): only the SERVE-side surface survives — build_tool_spec,
...
def build_tool_spec(messages, tools, iid="serve"):
    ...
    chunks = sorted(accumulated_chunks(steps, T - 1, read_lines=CHUNK_LINES)
                    ...
                    key=lambda c: c.step)
    # DASEIN-PATCH: step-0 fallback taken verbatim from the reference SERVING copy
