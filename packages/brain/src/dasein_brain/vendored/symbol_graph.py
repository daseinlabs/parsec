...
# DASEIN-PATCH(broken-grammar fallback): the language pack's prebuilt python grammar can fail to
...
def _py_defs_refs(src):
    ...
    parser, _ = _python_parser()   # DASEIN-PATCH: pack parser with wheel fallback (see above)
    if parser is None:
