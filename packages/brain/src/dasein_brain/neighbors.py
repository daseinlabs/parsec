...
Env dials:
  DASEIN_HOODS_PKL    — artifact path; unset = neighbors OFF (nf=None, exactly
  ...
  DASEIN_NEIGHBORS    — runaway-baseline cost top-k (default 16; reference
                        AC_NEIGHBORS)
  DASEIN_NEIGHBORS_X  — GNN block top-x (default 2 — the TRAINED value; do
...
class HoodsError(RuntimeError):
    """Artifact missing/corrupt while DASEIN_HOODS_PKL is set — refuse to serve
...
class ArtifactFetcher:
    ...
    def __init__(self, path: str | Path, x: int = 2, k: int = 16):
        ...
        if not p.is_file():
            raise HoodsError(
                f"hoods artifact MISSING at {p} (DASEIN_HOODS_PKL is set) — refusing to "
