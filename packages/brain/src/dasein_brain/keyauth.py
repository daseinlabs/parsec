"""Per-user `dsn_` key validation against the platform's /keys/validate seam.

DIRECTION §7 names this exact seam: "is this key entitled?" is what the brain
asks before scoring. Enabled by DASEIN_PLATFORM_URL — when set, the bearer token
...
Two properties keep this consistent with the rest of the brain:
  ...
  * FAIL OPEN, MEASURED: on a platform transport error we serve and count the
    ...
    DASEIN_BRAIN_AUTH_STRICT=1 to fail closed instead.
...
_TTL_OK_S = float(os.environ.get("DASEIN_BRAIN_AUTH_TTL_S", "300"))   # entitled: 5 min
_TTL_BAD_S = 30.0                                                     # denied: re-check sooner
_TIMEOUT_S = float(os.environ.get("DASEIN_BRAIN_AUTH_TIMEOUT_S", "3"))
...
def check(platform_url: str, key: str | None) -> bool:
    ...
    platform error unless DASEIN_BRAIN_AUTH_STRICT=1."""
    if not key:
    ...
    try:
        ...
        if os.environ.get("DASEIN_BRAIN_AUTH_STRICT") == "1":
