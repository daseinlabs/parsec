...
from .vendored.pyg_model import (attach_hetero, attach_steps, attach_task,
                                 attach_tools, edges)
...
def build_graph(nodes, task_emb, sys_emb, edges_supersession):
    """(xe fp32, xs, ei, et, n_own) — BYTE-MIRROR of TraceScorer._het_node_graph (which is
    itself the byte-mirror of trace_train.assemble_trace's HET path), from payload data.
    ...
    as the dev path casts the embedder output."""
    ...
    xe, xs, ei, et = attach_hetero(xe, xs, ei, et, chunks, n_own, ace, hde,
                                   {}, {}, ntype, sys_emb=sysv)
    return xe, xs, ei, et, n_own


def score_trace(bundle, nodes, task_emb, sys_emb, mask, decided_struct, edges_supersession):
    """(scores over nodes, tau, timings_ms) — the v1 twin of TraceScorer.score_trace: same
    forward, same never-cut 1.0 default on non-mask rows, same constant calibrated tau.
    decided_struct rides in from the client (49 cols, computed where the text lives)."""
    n_own = len(nodes)
    xe, xs, ei, et, _ = build_graph(nodes, task_emb, sys_emb, edges_supersession)
    ...
    return sc, bundle.calib_tau, {"embed": 0.0, "forward": round(forward_ms, 3)}
...
def score_tools(bundle, nodes, task_emb, sys_emb, tool_embs):
    ...
    xe, xs, ei, et, n_own = build_graph(nodes, task_emb, sys_emb, [])
