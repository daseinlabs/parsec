...
Level comes from DASEIN_BRAIN_LOG (default INFO); DASEIN_BRAIN_LOG_JSON=1
switches the handler to JSON lines. Timestamps are UTC. Loggers live under the
"dasein_brain" hierarchy and PROPAGATE (pytest caplog sees every record).
...
def _configure() -> logging.Logger:
    ...
    if getattr(root, "_dasein_configured", False):
        return root
    level = os.environ.get("DASEIN_BRAIN_LOG", "INFO").upper()
    root.setLevel(getattr(logging, level, logging.INFO))
    handler = logging.StreamHandler()
    if os.environ.get("DASEIN_BRAIN_LOG_JSON") == "1":
