"""parsec-brain — hosted scoring API (control plane, private).
...
reference scoring path (parsec_brain.vendored) so parity with the trainer
...
PARSEC_TARGET_COV — and any mismatch refuses to start (§8.2: brain-API score ==
...
unrepresentable by construction — parsec_brain.v1graph), dispatched on the
