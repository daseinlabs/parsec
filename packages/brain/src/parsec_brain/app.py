"""FastAPI surface for BOTH brain contracts, dispatched on the request's `contract` field:
  ...
  brain-api-dev/v0  (contracts/schemas/brain-api-dev.schema.json)
    ⚠️ Dev-raw posture: carries the internal message view (RAW TEXT) — only for a scoring
    host the user operates, behind the client's explicit PARSEC_BRAIN_DEV_RAW=1 opt-in.
...
def create_app() -> FastAPI:
    ...
    # lands in the container startup-probe window. Fail-open: a warm failure
