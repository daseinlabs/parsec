...
_DEFAULT_CKPT = "~/.parsec/brain/curator_v4_prod.pt"           # the 36MB .pt stays OUT of the repo
_EXPECTED_RULES = 16
...
def load_bundle(ckpt_path: str | None = None, rules_path: str | None = None,
    ...
    ckpt_file = Path(ckpt_path or os.environ.get("PARSEC_CKPT", _DEFAULT_CKPT)).expanduser()
    if not ckpt_file.is_file():
        raise BundleError(f"checkpoint not found: {ckpt_file} (set PARSEC_CKPT)")
    ...
    target_cov = os.environ.get("PARSEC_TARGET_COV", os.environ.get("AC_TARGET_COV", "0.70"))
    try:
        tbl = {float(k): float(v) for k, v in tau_table.items()}
        want = float(target_cov)
    except (TypeError, ValueError) as e:
        raise BundleError(f"tau_calib resolution failed for PARSEC_TARGET_COV={target_cov!r}: {e}")
    ...
    # PARSEC_SERVE_TAU: the reference's AC_SERVE_TAU dial (curator.py L99-104,
    ...
    serve_tau_override = os.environ.get("PARSEC_SERVE_TAU")
    if serve_tau_override:
        try:
            calib_tau = float(serve_tau_override)
        except ValueError as e:
            raise BundleError(f"PARSEC_SERVE_TAU={serve_tau_override!r}: {e}")
        log.info("PARSEC_SERVE_TAU override active: tau = %s", calib_tau)
    ...
    rules_file = Path(rules_path or os.environ.get("PARSEC_RULES_JSON",
    ...
    # ---- hoods artifact (cross-trace neighborhoods): PARSEC_HOODS_PKL unset = nf OFF, exactly
    ...
    hoods = None
    hoods_pkl = hoods_path or os.environ.get("PARSEC_HOODS_PKL")
    if hoods_pkl:
        try:
            hoods = ArtifactFetcher(
                hoods_pkl,
                x=int(os.environ.get("PARSEC_NEIGHBORS_X", "2")),   # trained value — do not change
                k=int(os.environ.get("PARSEC_NEIGHBORS", "16")))
        except (HoodsError, ValueError) as e:
            raise BundleError(f"PARSEC_HOODS_PKL={hoods_pkl}: {e}")
    else:
        log.info("neighbors OFF (PARSEC_HOODS_PKL unset) — nf=None, +3 zero block-parity cols")
