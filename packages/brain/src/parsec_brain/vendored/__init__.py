...
Provenance:
  ...
  trace_contract.py / embedding.py / parsec_embed.py — adaptive-context-clean/adaptive_context
...
'# PARSEC-PATCH:' comment:
  PATCH 1 (parsec_embed.py)  — JSON POST to PARSEC_EMBED_URL replaces k8s pod discovery +
                               binary /encode_bin; PARSEC_EMBED_BACKEND=hash (embedding.py's
                               deterministic backend) makes tests hermetic.
  PATCH 2 (scorer._het_readout, parsec_brain/scorer.py) — the readout ALWAYS emits all 49
...
`parsec_brain._flags` MUST be imported before anything here (chunking reads AC_CHUNK_MODE at
import time); the parent package __init__ enforces that for any `parsec_brain.*` import.
"""
