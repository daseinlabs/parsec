...
def train_identity(base_ckpt: Path, work_dir: Path) -> Path:
    ...
    print(
        "[trainer] IDENTITY TRAINER: returning the base GNN unchanged "
        f"({base_ckpt.name}) — no training was performed",
        flush=True,
    )
    return out
...
def promote(
    ...
    print(
        f"[trainer] promoted {version} -> {bundle_dir} "
        f"(checkpoint_id {manifest['checkpoint_id'][:12]}…, trained: false)",
        flush=True,
    )
...
def retrain(
    ...
    corpus_report = collect(corpus_dir)
    work = out_dir / "_work"
    candidate = train_identity(base_ckpt, work)
    eval_report = eval_gate(candidate, base_ckpt)
    version = version or f"identity-{_sha256(base_ckpt)[:12]}"
    result = promote(candidate, out_dir, version, base_ckpt, corpus_report, eval_report)
