...
- eval_gate(): REAL, not stubbed — the promoted candidate must pass the same
  self-validation the brain enforces at startup (parsec_brain.bundle) and
...
- promote(): versioned, immutable bundle dir + manifest (checkpoint_id =
  sha256 of the .pt), which `PARSEC_CKPT` can point the brain at directly.
...
def promote(
    ...
    (`PARSEC_CKPT=<bundle>/curator.pt`). Refuses to overwrite a version."""
    ...
    manifest = {
        ...
        "trained": False,
        "provenance": "identity-stub (parsec_trainer.retrain — no training performed)",
...
def main() -> None:
    ...
    ap.add_argument(
        "--base-ckpt",
        default=os.path.expanduser("~/.parsec/brain/curator_v4_prod.pt"),
