...
// [\s\x1c-\x1f]: Python re's \s includes the C0 separators that the
// regex crate's \p{White_Space} excludes (same set as pystr::py_is_space).
...
mod tests {
    ...
    fn parse_grep_candidate_parses_standard_grep_output() {
        let got = parse_grep_candidate("src/main.rs:42:fn main() {");

        assert_eq!(got, Some(("main.rs".to_string(), Some(42))));
    }

    #[test]
    fn parse_grep_candidate_preserves_content_with_multiple_colons() {
        let got = parse_grep_candidate("src/main.rs:42:let url = \"http://example.com:8080\";");

        assert_eq!(got, Some(("main.rs".to_string(), Some(42))));
    }

    #[test]
    fn parse_grep_candidate_accepts_missing_line_number() {
        let got = parse_grep_candidate("src/main.rs:some content");

        assert_eq!(got, Some(("main.rs".to_string(), None)));
    }

    #[test]
    fn parse_grep_candidate_accepts_empty_matching_text() {
        let got = parse_grep_candidate("src/main.rs:42:");

        assert_eq!(got, Some(("main.rs".to_string(), Some(42))));
    }

    #[test]
    fn parse_grep_candidate_rejects_non_numeric_line_number() {
        let got = parse_grep_candidate("src/main.rs:abc:matching text");

        assert_eq!(got, Some(("main.rs".to_string(), None)));
    }

    #[test]
    fn parse_grep_candidate_ignores_empty_lines() {
    ...
    #[test]
    fn parse_grep_candidate_ignores_reranked_lines() {
        assert_eq!(parse_grep_candidate("[reranked] src/main.rs:42:code"), None);
    }

    #[test]
    fn parse_grep_candidate_parses_python_file_path() {
        let got = parse_grep_candidate("src/utils.py:10:def hello():");

        assert_eq!(got, Some(("utils.py".to_string(), Some(10))));
    }

    #[test]
    fn parse_grep_candidate_handles_large_line_numbers() {
        let got = parse_grep_candidate("a.py:12345678901234567890:huge");

        assert_eq!(got, Some(("a.py".to_string(), Some(i64::MAX))));
    }
    ...
    #[test]
    fn sed_base_parses_range_command() {
    ...
    #[test]
    fn sed_base_parses_single_line_command() {
        assert_eq!(sed_base("sed -n 15p file.rs"), 15);

        assert_eq!(sed_base("sed -n 25p file.rs"), 25);
    }

    #[test]
    fn sed_base_defaults_to_one_when_no_line_number_exists() {
        assert_eq!(sed_base("cat file.rs"), 1);

        assert_eq!(sed_base("sed file.rs"), 1);
    }
    ...
    #[test]
    fn sed_base_handles_non_numeric_input_by_defaulting_to_one() {
        assert_eq!(sed_base("sed -n 'abc,xyzp' file.rs"), 1);
    }

    #[test]
    fn sed_base_saturates_large_line_numbers() {
        assert_eq!(
            sed_base("sed -n '99999999999999999999,100p' f.py"),
            i64::MAX
        );
    }
    ...
    #[test]
    fn chunk_new_derives_tokens_from_character_length() {
        let chunk = Chunk::new("abcdefgh", None, None, None, 1, "other");

        assert_eq!(chunk.text, "abcdefgh");
        assert_eq!(chunk.tokens, 2);
        assert_eq!(chunk.step, 1);
        assert_eq!(chunk.kind, "other");
    }

    #[test]
    fn chunk_new_uses_at_least_one_token() {
        let chunk = Chunk::new("", None, None, None, 1, "other");

        assert_eq!(chunk.tokens, 1);
    }

    #[test]
    fn chunk_new_counts_unicode_characters_for_tokens() {
        let chunk = Chunk::new("😀😀😀😀", None, None, None, 1, "other");
    ...
    #[test]
    fn with_tokens_only_replaces_positive_values() {
        let chunk = Chunk::new("hello", None, None, None, 1, "other");

        assert_eq!(chunk.clone().with_tokens(10).tokens, 10);

        assert_eq!(chunk.clone().with_tokens(0).tokens, 1);

        assert_eq!(chunk.with_tokens(-5).tokens, 1);
    }

    // ============================================================
    // chunk_observation - grep
    // ============================================================

    #[test]
    fn chunk_observation_creates_grep_chunks() {
        let obs = "src/a.rs:10:first match\nsrc/a.rs:20:second match";

        let chunks = chunk_observation("grep -n foo src/a.rs", obs, 1, 40, None, ChunkMode::Fixed);

        assert_eq!(chunks.len(), 2);

        assert_eq!(chunks[0].text, "src/a.rs:10:first match");
        ...
        assert_eq!(chunks[1].text, "src/a.rs:20:second match");

        assert_eq!(chunks[1].kind, "grep");

        assert_eq!(chunks[1].file, Some("a.rs".to_string()));

        assert_eq!(chunks[1].lo, Some(20));

        assert_eq!(chunks[1].hi, Some(20));
    }

    #[test]
    fn chunk_observation_adds_command_return_code_and_head() {
        let obs = "<returncode> 0 </returncode>\nhello world";

        let chunks = chunk_observation("echo hello", obs, 7, 40, None, ChunkMode::Fixed);
        ...
        for chunk in chunks {
            ...
            assert_eq!(chunk.rc, Some(0));
            assert!(chunk.head.starts_with("<returncode>"));
        }
    }

    // ============================================================
    // chunk_observation - read
    // ============================================================

    #[test]
    fn chunk_observation_chunks_read_output_by_window() {
        let obs = "line1\nline2\nline3\nline4\nline5";

        let chunks = chunk_observation("cat file.rs", obs, 2, 2, None, ChunkMode::Fixed);
    ...
    #[test]
    fn chunk_observation_uses_sed_start_line() {
        let obs = "line10\nline11\nline12";

        let chunks =
            chunk_observation("sed -n '10,12p' file.rs", obs, 1, 2, None, ChunkMode::Fixed);
    ...
    #[test]
    fn chunk_assistant_chunks_text_by_window() {
        let txt = "one\ntwo\nthree\nfour";

        let chunks = chunk_assistant(txt, 3, 2);
        ...
        assert_eq!(chunks[1].step, 3);
    }

    #[test]
    fn chunk_assistant_handles_single_line_text() {
        let chunks = chunk_assistant("hello world", 1, 40);
    ...
    #[test]
    fn chunk_observation_preserves_unicode_text() {
        let obs = "你好\n世界\n😀";

        let chunks = chunk_observation("echo test", obs, 1, 1, None, ChunkMode::Fixed);
    ...
    #[test]
    fn chunk_assistant_preserves_full_content() {
    ...
    #[test]
    fn chunk_observation_preserves_single_line_content() {
        let original = "this is a long single line with no newline characters";

        let chunks = chunk_observation("echo test", original, 1, 40, None, ChunkMode::Fixed);
