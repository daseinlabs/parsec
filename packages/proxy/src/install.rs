...
/// Opt-out for the install ping: `PARSEC_INSTALL_REPORT=0` (or the
/// cross-tool `DO_NOT_TRACK=1` convention, https://consoledonottrack.com).
/// Checked on every send so a user can flip it without restarting anything.
/// Only the ping is affected — the savings ledger still needs a key to ship
/// and is governed by `PARSEC_PLATFORM_URL` / `parsec key` as before.
pub fn opted_out() -> bool {
    ...
    fn on(v: &str) -> bool {
        !v.trim().is_empty() && !off(v)
    }
    std::env::var("PARSEC_INSTALL_REPORT")
        .map(|v| off(&v))
        .unwrap_or(false)
...
/// Everything a report needs, resolved live — None when the user opted out,
/// the platform URL is unconfigured, the state can't persist, or the last
/// report still stands.
