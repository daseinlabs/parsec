...
pub fn key_set(key: String, platform_url: Option<String>) -> anyhow::Result<()> {
    ...
    let masked = save_account_key(&key, platform_url)?;
    println!(
        "saved API key {masked} to {} — savings now report to your dashboard on the next \
...
/// Store the account key (and optional platform URL) and re-report the
/// install. The one write path for both `parsec login` and `parsec key set`,
/// so the two can never drift on where the key lands or what it triggers.
/// Returns the masked key for confirmation lines. Blocking (file + HTTP).
pub fn save_account_key(key: &str, platform_url: Option<String>) -> anyhow::Result<String> {
