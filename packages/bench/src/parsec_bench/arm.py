...
Port of adaptive-context-clean/bench/arm.py, adapted to the parsec topology.
...
varies is HOW the prompt is compressed before it reaches the model. Patterns:
  ...
  LocalProxyArm — the runner asks the arm to START a local proxy per solve
                  ...
                  landed (`ledger_path`). This is the parsec arm's shape: the
...
Concrete arm classes live in `parsec_bench.arms` (one module per arm); they
...
class Arm(abc.ABC):
    ...
    Identity / capability surface every arm MUST expose:
      name  : str        — registry key (e.g. "baseline", "parsec")
...
# ── local proxy spawned per solve (the parsec pattern) ────────────────────────
