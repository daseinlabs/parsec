...
fields the parsec proxy's ledger provides (see parsec_bench.ledger).
...
fields line up with parsec_bench.pricing.CostBreakdown so a record can be
...
class RunRecord:
    ...
    Savings (§8.4 — parsec-arm only, read from the proxy's savings ledger):
    ...
    # ── savings (§8.4, from the parsec proxy ledger; see parsec_bench.ledger) ──
