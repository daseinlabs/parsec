...
So we interpose ONE gateway at the BOTTOM of every chain:
    ...
    dasein   : Claude Code ──> dasein proxy (curates; its DASEIN_UPSTREAM is the
...
The dasein arm's SAVINGS accounting does not come from here — it comes from
the proxy's own ~/.dasein/ledger.jsonl (see dasein_bench.ledger), which holds
...
from dasein_bench.schema import CallUsage
...
# usage row into the right per-run JSONL. The dasein proxy reads the SAME
...
class UsageGateway:
    ...
    Usage:
        ...
        base_url = gw.base_url          # set ANTHROPIC_BASE_URL / the arm
                                        # proxy's DASEIN_UPSTREAM to this
        ...
        gw.stop()
