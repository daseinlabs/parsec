...
Port of adaptive-context-clean/bench/cc_runner.py, adapted to the dasein
...
rides the chain untouched — the gateway and the dasein proxy both forward auth
headers verbatim and never store them.
    ...
    dasein (LocalProxyArm)
        Claude Code ── ANTHROPIC_BASE_URL=proxy ──> dasein proxy (curates,
            writes the savings ledger; DASEIN_UPSTREAM = gateway)
            ──> gateway ──> upstream
        ...
        per-run ledger provides the §8.4 counterfactual (dasein_bench.ledger).
...
ANTHROPIC_CUSTOM_HEADERS (the dasein proxy forwards it upstream — server.rs
...
and `import dasein_bench.cc_runner` work on a box without them installed.
...
CUT vs the reference (clearly-marked seams):
  ...
  * The shared standalone gateway (CCB_GATEWAY_URL) — per-solve ephemeral
    ...
    gateway CAN isolate the dasein arm's rows per run; the per-solve topology
    is kept for simplicity, not necessity.)
  * The Vertex bridge (the reference gateway's MODE_VERTEX) — see
    dasein_bench.usage_gateway.
"""
...
# arms self-register on import; the registry lives in dasein_bench.arm.
import dasein_bench.arms  # noqa: F401  (import side effect: registers every arm)
from dasein_bench import ledger as ledger_mod
from dasein_bench.arm import Arm, ArmKind, RunContext, available_arms, get_arm
from dasein_bench.grader import SWEBenchGrader
from dasein_bench.pricing import price_run, rates_for
from dasein_bench.schema import RunRecord
from dasein_bench.usage_gateway import DEFAULT_UPSTREAM, RUN_ID_HEADER, UsageGateway
...
async def _run_sdk(
    *,
    ...
    # the gateway tags usage rows by it, and the dasein proxy prefixes its
...
def run_agent(
    ...
    *,
    ...
    # arriving via the dasein proxy (which strips the run-id header upstream).
    ...
    # §8.4 savings: the arm proxy's ledger (dasein arm), summarized with null
...
# ── driver ─────────────────────────────────────────────────────────────────────
def main() -> None:
    ap = argparse.ArgumentParser(description="dasein-bench Claude Code runner")
...
def _GraderDefault(field: str) -> str:
    ...
    from dasein_bench import grader as _g
