...
Arms:
  baseline — the dasein binary WITHOUT a brain: passthrough-shaped curation
             (wire freeze + ledger machinery live, no cuts). Deltas ≈ 0.
  dasein   — the same binary with DASEIN_BRAIN_URL (+ dev-raw opt-in): the
             real-scorer path trims observations and prunes tools.

CLI:
  python -m dasein_bench.replay --fixture <recorded.json> [--brain-url URL]
"""
...
from dasein_bench.proxy_bin import ProxyProcess, repo_root
...
class ReplayReport:
    ...
    fixture: str
    arm: str                              # "dasein" (brain wired) or "baseline"
...
def replay(fixture: str | Path, *, brain_url: Optional[str] = None,
    ...
    own_tmp = None
    if work_dir is None:
        own_tmp = tempfile.TemporaryDirectory(prefix="dasein_replay_")
    ...
    rep = ReplayReport(fixture=str(fixture),
                       arm="dasein" if brain_url else "baseline", turns=[])
...
def main(argv: Optional[list] = None) -> int:
    ...
    ap.add_argument("--brain-url", default="",
                    help="brain URL -> replays the DASEIN arm (real-scorer "
                         "path); empty -> baseline (passthrough machinery)")
    ap.add_argument("--proxy-bin", default="",
                    help="dasein binary (default: $DASEIN_BIN / target/{release,debug})")
