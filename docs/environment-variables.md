...
| `DASEIN_API_KEY` | unset | The user's per-account `dsn_` key (minted in the dashboard). When set alongside a platform URL, each savings-ledger row is ALSO shipped to the platform's `/ledger` (fire-and-forget, fail-open) for per-user attribution in the dashboard. Unset ⇒ local-only ledger. **Primary path is `dasein key set <dsn_…>`** (or the `/dasein-key` skill), which stores the key in `~/.dasein/credentials.json` (0600) and takes effect next request; this env var overrides that file when set. |
